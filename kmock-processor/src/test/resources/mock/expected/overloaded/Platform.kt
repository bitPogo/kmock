...
internal class PlatformMock(
    ...
    LazyThreadSafetyMode.PUBLICATION) {
        if (spyOn == null) {
            PropertyProxy("mock.template.overloaded.PlatformMock#_hashCode", spyOnGet = null,
    ...
    kotlin.Any> by lazy(mode = LazyThreadSafetyMode.PUBLICATION) {
        SyncFunProxy("mock.template.overloaded.PlatformMock#_fooWithAnyString", spyOn = if (spyOn
            != null) { { fuzz, ozz ->
            foo(fuzz, ozz) } } else { null }, collector = verifier, freeze = freeze, relaxer = null)
    }
    ...
    kotlin.Any> by lazy(mode = LazyThreadSafetyMode.PUBLICATION) {
        SyncFunProxy("mock.template.overloaded.PlatformMock#_fooWithStringAny", spyOn = if (spyOn
            != null) { { fuzz, ozz ->
            foo(fuzz, ozz) } } else { null }, collector = verifier, freeze = freeze, relaxer = null)
    }
    ...
    lazy(mode = LazyThreadSafetyMode.PUBLICATION) {
        SyncFunProxy("mock.template.overloaded.PlatformMock#_fooWithAny", spyOn = if (spyOn !=
            null) { { fuzz ->
    ...
    public val _fooWithMockTemplateOverloadedPlatform:
        KMockContract.SyncFunProxy<Unit, (mock.template.overloaded.Platform) -> kotlin.Unit> by
    lazy(mode = LazyThreadSafetyMode.PUBLICATION) {
    ...
    LazyThreadSafetyMode.PUBLICATION) {
        SyncFunProxy("mock.template.overloaded.PlatformMock#_fooWithMockTemplateOverloadedLPG",
            spyOn = if (spyOn != null) { { fuzz ->
                foo(fuzz) } } else { null }, collector = verifier, freeze = freeze, unitFunRelaxer = if
                                                                                                         (relaxUnitFun) { { relaxVoidFunction() } } else { null }, relaxer = null, buildInRelaxer =
            null)
