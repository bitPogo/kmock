...
internal class CommonMock(
    ...
    LazyThreadSafetyMode.PUBLICATION) {
        if (spyOn == null) {
            PropertyProxy("mock.template.overloaded.CommonMock#_hashCode", spyOnGet = null,
                spyOnSet = null, collector = verifier, freeze = freeze, relaxer = null)
        } else {
            PropertyProxy("mock.template.overloaded.CommonMock#_hashCode", spyOnGet = {
    ...
    kotlin.Any> by lazy(mode = LazyThreadSafetyMode.PUBLICATION) {
        SyncFunProxy("mock.template.overloaded.CommonMock#_fooWithAnyString", spyOn = if (spyOn !=
            null) { { fuzz, ozz ->
            foo(fuzz, ozz) } } else { null }, collector = verifier, freeze = freeze, relaxer = null)
    }
    ...
    kotlin.Any> by lazy(mode = LazyThreadSafetyMode.PUBLICATION) {
        SyncFunProxy("mock.template.overloaded.CommonMock#_fooWithStringAny", spyOn = if (spyOn !=
            null) { { fuzz, ozz ->
            foo(fuzz, ozz) } } else { null }, collector = verifier, freeze = freeze, relaxer = null)
    }
    ...
    public val _fooWithMockTemplateOverloadedCommon:
        KMockContract.SyncFunProxy<Unit, (mock.template.overloaded.Common) -> kotlin.Unit> by
    lazy(mode = LazyThreadSafetyMode.PUBLICATION) {
        SyncFunProxy("mock.template.overloaded.CommonMock#_fooWithMockTemplateOverloadedCommon",
