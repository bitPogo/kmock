...
internal class SharedMock(
    ...
    LazyThreadSafetyMode.PUBLICATION) {
        if (spyOn == null) {
            ...
            PropertyProxy("mock.template.buildIn.SharedMock#_foo", spyOnGet = { spyOn.foo },
    ...
    public val _bar: KMockContract.PropertyProxy<Int> by lazy(mode = LazyThreadSafetyMode.PUBLICATION)
    {
        if (spyOn == null) {
            ...
            PropertyProxy("mock.template.buildIn.SharedMock#_bar", spyOnGet = { spyOn.bar },
                collector = verifier, freeze = freeze, relaxer = null)
        }
    }
    ...
    LazyThreadSafetyMode.PUBLICATION) {
        if (spyOn == null) {
            PropertyProxy("mock.template.buildIn.SharedMock#_buzz", spyOnGet = null, spyOnSet =
            null, collector = verifier, freeze = freeze, relaxer = null)
        } else {
            PropertyProxy("mock.template.buildIn.SharedMock#_buzz", spyOnGet = { spyOn.buzz },
