...
All artefacts are currently only consumable as snapshots.
Also changes may fly in on short notice. Currently the short hash of the latest commit of the main branch is used instead of a proper version. You will find the hash directly on the link:{link-repository}[start page] under the `Go to file` button.
