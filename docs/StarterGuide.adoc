...
As already shown in the examples above KMock comes with a factory function `kmock` and should be used instead of instantiating the Mocks by hand.
...
You can enable also interfaces as types for `kmock`.
This is meant for relaxing to ease the need to add boilerplate code.
To enable interfaces you simply set `allowInterfacesOnKmock` of KMock's Gradle Plugin to `true`.
...
This will add the Mock to the factory function `kspy`. +
...
This is meant for relaxing to ease the need to add boilerplate code.
To enable interfaces you simply set `allowInterfacesOnKspy` of KMock's Gradle Plugin to `true`.
