...
IMPORTANT: You must set the root package of your project in the Extentsion of the KMock's Gradle Plugin via the `rootPackage` property.
...
KMock also allows you to stub/mock build-in methods like `toString`.
Use them with caution and only if the intended implementation of the mocked Interface requires/has special behaviour.
...
WARNING: Spying is not currently supported for recursive generics on function level. For example `fun <T: Comparable<T>> doSomething()` will result in an error if mocked and invoked.
...
fun sampleTest() {
    ...
    assertTrue(someInstance.equals(subjectToSpyOn)) // will pass
    assertTrue(subjectToSpyOn.equals(someInstance)) // will fail
    assertTrue(someInstance.equals(someInstance)) // will fail
}

----

While the first Assertion passes the others won't.
So why works it that way? +
KMock is as mentioned currently only build on top of Interface therefore the 2nd assertion fails unless `AnyImplementation` has a custom implementation of `equals`.
The 3rd assertion fails since the Proxy of `equals` utilizes the implementation of `AnyImplementation` due to Spying. +
Since this an undesirable behaviour expect changes rather in the future.
